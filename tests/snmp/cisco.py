...
__all__ = ['TestSNMPCisco']


class TestSNMPCisco(unittest.TestCase):

    def setUp(self):
        self.host = settings['cisco-snmp']['host']
        self.community = settings['cisco-snmp']['community']
        self.port = settings['cisco-snmp'].get('port', 161)

        self.device = Cisco(self.host, self.community, self.port)
